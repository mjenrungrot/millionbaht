...
def save_audio_file(audio_bytes: bytes, filename: str = "output.mp3") -> None:
...
def tiktok_tts(text: str, voice: str = "none") -> Optional[bytes]:
    ...
    try:
        if len(text) < TEXT_BYTE_LIMIT:
            ...
            if current_endpoint == 0:
                audio_base64_data = str(audio_bytes).split('"')[5]
            else:
                audio_base64_data = str(audio_bytes).split('"')[3].split(",")[1]
            ...
            audio_base64_data_tmp: list[Optional[str]] = [None] * len(text_parts)
            ...
            def generate_audio_thread(text_part, index):
                ...
                audio_base64_data_tmp[index] = base64_data
            ...
            audio_base64_data = "".join(audio_base64_data_tmp)  # type: ignore
            assert isinstance(audio_base64_data, str)
...
if __name__ == "__main__":
    ...
    voice = "en_us_006"
    audio_base64_data_bytes = tiktok_tts(text, voice)
    assert audio_base64_data_bytes is not None
    save_audio_file(audio_base64_data_bytes, "output.mp3")
    print("Audio file has been successfully created")
