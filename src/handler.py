...
class ProcResponse(BaseModel):
    ...
    def fail(cls, err: Exception) -> "ProcResponse":
        return ProcResponse(success=False, path=Path(), message=f"{type(err)}: {str(err)}")
...
class SongQueue:
    ...
    def __init__(self, bot: commands.Bot, guild_id: int):
        ...
        if False:
            ...
            if voice_client is not None and not voice_client.is_playing():
                voice_client.play(
                    FFmpegOpusAudio(str(random.choice(list(Constants.OPENNING_OUTDIR.iterdir())))),
    ...
    async def process_song(self, song: SongRequest):
        ...
        if song.channel is not None:
            await song.channel.send(f'Looking for "{song.proc_request.query}" ...')
    ...
    def validate(self):
        while self.last_processed.state in [State.Done, State.AwaitingPlay] and self.last_processed.next is not None:
        ...
        if voice_client is not None and not voice_client.is_playing():
            if self.last_played.state == State.AwaitingPlay:
                ...
                if response.success:
                    self.loop.create_task(to_play.channel.send(f"Now playing: {response.url}"))
                    def after(exception):
                ...
                if not self.queue_empty_played:
                    voice_client.play(
                        FFmpegOpusAudio(str(random.choice(list(Constants.EMPTY_OUTDIR.iterdir())))),
                        after=lambda x: self.validate(),
                    )
                    self.queue_empty_played = True
